...
def get_db():
    ...
    try: conn.execute("ALTER TABLE products ADD COLUMN color TEXT DEFAULT ''"); conn.commit()
    except: pass
    return conn
...
def sync_from_pos():
    ...
    for p in data["products"]:
        ...
        c.execute("""INSERT OR REPLACE INTO products
                     (barcode,name,brand,category,price,stock,image_url,sizes,gender,web_group,custom_fields,is_active,updated_at,shop_source,color)
                     VALUES (?,?,?,?,?,?,?,?,?,?,?,1,?,?,?)""",
...
def delete_all_orders():
    ...
    try:
        conn = get_db()
        try: conn.execute("DELETE FROM order_items")
        except: pass
...
def delete_single_order(order_no):
    ...
    try:
        conn = get_db()
        try: conn.execute("DELETE FROM order_items WHERE order_no=?", (order_no,))
        except: pass
