...
def init_db():
    ...
    for col in ["image_url","sizes","gender","shop_source","web_group","discount_pct"]:
...
def sync_from_pos():
    ...
    for p in data["products"]:
        c.execute("""INSERT OR REPLACE INTO products
                     (barcode,name,brand,category,price,stock,image_url,sizes,gender,web_group,is_active,updated_at,shop_source)
                     VALUES (?,?,?,?,?,?,?,?,?,?,1,?,?)""",
                  (p.get("barcode",""),p.get("name",""),p.get("brand",""),p.get("category",""),
                   float(p.get("price",0)),int(p.get("stock",0)),p.get("image_url",""),
                   p.get("sizes",""),p.get("gender","Unisex"),p.get("web_group",""),now,SHOP_SOURCE))
        updated += 1
